...
const EditProfileScreen = ({ onBack, isInitialSetup = false }: { onBack: () => void, isInitialSetup?: boolean }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Don't use aggressive timeout here — allow updateUser/upsert to finish
            ...
            if (password) {
                ...
                // await the updateUser directly (no short timeout)
                const { data: updateData, error: pwdError } = await supabase.auth.updateUser({ password });
                console.log('[Profile] updateUser returned', { updateData, pwdError });
                if (pwdError) {
